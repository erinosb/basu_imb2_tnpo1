...
// AUTHOR:   Sam Zavislan, Erin Osborne Nishimura
// SCRIPT:   quantifyPlasmaMembraneRegion_260629.ijm
...
inputDir="/Users/ambikabasu/Nishimura_lab/Macros/QuantificationNuclearMembrane /U2OS/231206_imaging_U2OS/01_input";
...
outputDir="/Users/ambikabasu/Nishimura_lab/Macros/QuantificationNuclearMembrane /U2OS/231206_imaging_U2OS/03_output";
...
// * Default is to look for R3D.dv files. To modify, edit line 195
...
// Things to test
  ...
  // 1) do results in this code mirror (or are similar to) past runs?
  // 2) if you hard code in the middle slice #, the rectangle metrics, are the results identical to past runs?
  // 3) test with a 3-channel image
  // 4) test with specific values for max/min in each channel v. setting to 0
  // 5) 

////////////////////////////////////////////
///     BUGS, TOPICS, IDEAS COMPLETE     ///
////////////////////////////////////////////
...
logPath = outputDir + "/" + todaysDate + "_" + timeNow + "_" + "logfile" + "_" + "for" + "_" + folderName + ".txt";
// Automatically append folder name to "for" + "_" +
dataPath = outputDir + "/" + todaysDate + "_" + timeNow + "_" + "datafile" + "_" + "for" + "_" + folderName + ".txt";

// Record time
logfile = File.open(logPath);
print("Initiaiting macro: quantifyPlasmaMembraneRegion");
File.append("Initiaiting macro: quantifyPlasmaMembraneRegion", logPath);
print("\nDate: " + todaysDate);
print("Time: " + timeNow);
...
for (i=0; i<dv_array.length; i++) {
    ...
    waitForUser("Action Required", "By default, 5 slices will be selected for Z-projection. \nPlease look through the image and select the middle slice. \nClick OK when ready.");
    ...
    do {
    	...
    	rectLength = 180; // long dimension, same as original rectangle width
    	rectThickness = 50; // short dimension, same as original rectangle height
	...
	Stack.setChannel(1);
	roiManager("Add");
	ch1RoiIndex = roiManager("count") - 1; // index of the rotated rectangle just added, valid regardless of how many ROIs have accumulated across loop iterations
	run("Plot Profile");
	...
	// Re-select the SAME rotated rectangle ROI used for Channel 1 (captured as
	// ch1RoiIndex above), rather than rebuilding from x,y,width,height, since those
	...
	// position - since this ROI was first added while Channel 1 was active, selecting
	// it silently switches the window back to Channel 1. Stack.setChannel(2) MUST run
	// AFTER this select (not before), or Plot Profile below ends up measuring Channel 1
	// again instead of Channel 2, producing a duplicate of the ch1 row in the datafile.
	roiManager("Select", ch1RoiIndex);
	Stack.setChannel(2);
